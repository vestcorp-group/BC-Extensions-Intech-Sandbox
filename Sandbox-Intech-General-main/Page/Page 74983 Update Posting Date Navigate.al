Page 74983 "Update Posting Date Navigate"
{
    // --------------------------------------------------------------------------------------------------
    ...
    layout
    {
        area(content)
        {
            group(Document)
            {
                ...
                field(UpdateDate_gDate; UpdateDate_gDate)
                {
                    ApplicationArea = Basic;
                    Caption = 'Update Date';
                }
    ...
    var
        ...
        Text095984: label 'There are no posted records with this document number.';
        ...
        Text0005898: label 'The search results in too many external documents. Please use Navigate from the relevant ledger entries.';
    ...
    begin
        ...
        repeat
            case Rec."Table ID" of
                    ...
                    begin
                        if SalesCrMemoHeader.ReadPermission then begin
                            ...
                            if SalesCrMemoHeader.FindFirst then begin
                                SalesCrMemoHeader."Posting Date" := UpdateDate_gDate;
                                ...
                                if SalesCrMemoLine_lRec.FindFirst then begin
                                    repeat
                                        if SalesCrMemoLine_lRec."Posting Date" <> 0D then begin
                                            SalesCrMemoLine_lRec."Posting Date" := UpdateDate_gDate;
                    ...
                    begin
                        if PurchInvHeader.ReadPermission then begin
                            ...
                            if PurchInvHeader.FindFirst then begin
                                PurchInvHeader."Posting Date" := UpdateDate_gDate;
                                ...
                                if PurchInvLine_gRec.FindFirst then begin
                                    repeat
                                        if PurchInvLine_gRec."Posting Date" <> 0D then
                                            PurchInvLine_gRec."Posting Date" := UpdateDate_gDate;
                    ...
                    begin
                        if R6650_lRec.ReadPermission then begin
                            ...
                            if R6650_lRec.FindFirst then begin
                                R6650_lRec."Posting Date" := UpdateDate_gDate;
                                R6650_lRec."Posting Date" := UpdateDate_gDate;
                                R6650_lRec.Modify;
                                ...
                                if R6651_lRec.FindFirst then begin
                                    repeat
                                        if R6651_lRec."Posting Date" <> 0D then
                                            R6651_lRec."Posting Date" := UpdateDate_gDate;
                                        R6651_lRec.Modify;
                    ...
                    begin
                        if R6660_lRec.ReadPermission then begin
                            ...
                            if R6660_lRec.FindFirst then begin
                                R6660_lRec."Posting Date" := UpdateDate_gDate;
                                R6660_lRec."Posting Date" := UpdateDate_gDate;
                                R6660_lRec.Modify;
                                ...
                                if R6661_lRec.FindFirst then begin
                                    repeat
                                        if R6661_lRec."Posting Date" <> 0D then
                                            R6661_lRec."Posting Date" := UpdateDate_gDate;
                                        R6661_lRec.Modify;
                    ...
                    begin
                        if PurchRcptHeader.ReadPermission then begin
                            ...
                            if PurchRcptHeader.FindFirst then begin
                                PurchRcptHeader."Posting Date" := UpdateDate_gDate;
                                ...
                                if PurchRcptLine_gRec.FindFirst then begin
                                    repeat
                                        if PurchRcptLine_gRec."Posting Date" <> 0D then begin
                                            PurchRcptLine_gRec."Posting Date" := UpdateDate_gDate;
                    ...
                    begin
                        if ItemLedgerEntry_gRec.ReadPermission then begin
                            ...
                            if ItemLedgerEntry_gRec.FindFirst then begin
                                repeat
                                    ItemLedgerEntry_gRec."Posting Date" := UpdateDate_gDate;
                    ...
                    begin
                        if WarehouseEntry_gRec.ReadPermission then begin
                            ...
                            if WarehouseEntry_gRec.FindSet then begin
                                repeat
                                    WarehouseEntry_gRec."Registering Date" := UpdateDate_gDate;
                    ...
                    begin
                        if SalesInvHeader.ReadPermission then begin
                            ...
                            if SalesInvHeader.FindFirst then begin
                                SalesInvHeader."Posting Date" := UpdateDate_gDate;
                                ...
                                if SalesInvLine_gRec.FindFirst then begin
                                    repeat
                                        if SalesInvLine_gRec."Posting Date" <> 0D then begin
                                            SalesInvLine_gRec."Posting Date" := UpdateDate_gDate;
                ...
                124:
                    begin
                        if PurchCrmemohdr_gRec.ReadPermission then begin
                            ...
                            if PurchCrmemohdr_gRec.FindFirst then begin
                                PurchCrmemohdr_gRec."Posting Date" := UpdateDate_gDate;
                                ...
                                if PurchCrmemoLine_gRec.FindFirst then begin
                                    repeat
                                        if PurchCrmemoLine_gRec."Posting Date" <> 0D then begin
                                            PurchCrmemoLine_gRec."Posting Date" := UpdateDate_gDate;
                                            PurchCrmemoLine_gRec.Modify;
                                        end;
                                    until PurchCrmemoLine_gRec.Next = 0;
                                    PurchCrmemohdr_gRec."Document Date" := UpdateDate_gDate;
                    ...
                    begin
                        if GLEntry.ReadPermission then begin
                            ...
                            if GLEntry.FindSet then begin
                                repeat
                                    GLEntry."Posting Date" := UpdateDate_gDate;
                                    GLEntry.Modify;
                    ...
                    begin
                        if TDSEntry.ReadPermission then begin
                            ...
                            if TDSEntry.FindFirst then begin
                                repeat
                                    TDSEntry."Posting Date" := UpdateDate_gDate;
                                    TDSEntry.Modify;
                    ...
                    begin
                        if VendLedgEntry.ReadPermission then begin
                            ...
                            if VendLedgEntry.FindFirst then begin
                                repeat
                                    VendLedgEntry."Posting Date" := UpdateDate_gDate;
                    ...
                    begin
                        if DtldVendLedgEntry.ReadPermission then begin
                            ...
                            if DtldVendLedgEntry.FindFirst then begin
                                repeat
                                    DtldVendLedgEntry."Posting Date" := UpdateDate_gDate;
                    ...
                    begin
                        if CustLedgEntry.ReadPermission then begin
                            ...
                            if CustLedgEntry.FindFirst then begin
                                repeat
                                    CustLedgEntry."Posting Date" := UpdateDate_gDate;
                    ...
                    begin
                        if DtldCustLedgEntry.ReadPermission then begin
                            ...
                            if DtldCustLedgEntry.FindFirst then begin
                                repeat
                                    DtldCustLedgEntry."Posting Date" := UpdateDate_gDate;
                    ...
                    begin
                        if ValueEntry.ReadPermission then begin
                            ...
                            if ValueEntry.FindFirst then begin
                                repeat
                                    ValueEntry."Posting Date" := UpdateDate_gDate;
                                    ValueEntry.Modify;
                    ...
                    begin
                        if CapacityLdrEntry_gRec.ReadPermission then begin
                            ...
                            if CapacityLdrEntry_gRec.FindFirst then begin
                                repeat
                                    CapacityLdrEntry_gRec."Posting Date" := UpdateDate_gDate;
                    ...
                    begin
                        if BankAccLedgEntry.ReadPermission then begin
                            ...
                            if BankAccLedgEntry.FindFirst then begin
                                repeat
                                    BankAccLedgEntry."Posting Date" := UpdateDate_gDate;
                    ...
                    begin
                        if CheckLedgEntry.ReadPermission then begin
                            ...
                            if CheckLedgEntry.FindFirst then begin
                                repeat
                                    CheckLedgEntry."Posting Date" := UpdateDate_gDate;
                        ...
                        if GSTLedgerEntry.ReadPermission then begin
                            ...
                            if GSTLedgerEntry.FindFirst then begin
                                repeat
                                    ...
                                    Mod18005."Posting Date" := UpdateDate_gDate;
                                    Mod18005.Modify;
                        ...
                        if DetailedGSTLedgerEntry.ReadPermission then begin
                            ...
                            if DetailedGSTLedgerEntry.FindFirst then begin
                                repeat
                                    ...
                                    Mod18001."Posting Date" := UpdateDate_gDate;
                                    Mod18001.Modify;
                    ...
                    begin
                        if PostedAssemblyHeader.ReadPermission then begin
                            ...
                            if PostedAssemblyHeader.FindFirst then begin
                                PostedAssemblyHeader."Posting Date" := UpdateDate_gDate;
                    ...
                    begin
                        if T5744_lRec.ReadPermission then begin
                            ...
                            if T5744_lRec.FindFirst then begin
                                T5744_lRec."Posting Date" := UpdateDate_gDate;
                    ...
                    begin
                        if T5746_lRec.ReadPermission then begin
                            ...
                            if T5746_lRec.FindFirst then begin
                                T5746_lRec."Posting Date" := UpdateDate_gDate;
                    ...
                    begin
                        if ServiceLedgerEntry_lRec.ReadPermission then begin
                            ...
                            if ServiceLedgerEntry_lRec.FindFirst then begin
                                repeat
                                    if ServiceLedgerEntry_lRec."Posting Date" <> UpdateDate_gDate then begin
                                        ServiceLedgerEntry_lRec."Posting Date" := UpdateDate_gDate;
                    ...
                    begin
                        if ServiceInvoiceHeader_lRec.ReadPermission then begin
                            ...
                            if ServiceInvoiceHeader_lRec.FindFirst then begin
                                ServiceInvoiceHeader_lRec."Posting Date" := UpdateDate_gDate;
                                ...
                                if ServiceInvoiceLine_lRec.FindFirst then begin
                                    repeat
                                        if ServiceInvoiceLine_lRec."Posting Date" <> 0D then begin
                                            ServiceInvoiceLine_lRec."Posting Date" := UpdateDate_gDate;
                    ...
                    begin
                        if TCSEntry.ReadPermission then begin
                            ...
                            if TCSEntry.FindFirst then begin
                                repeat
                                    if TCSEntry."Posting Date" <> UpdateDate_gDate then begin
                                        TCSEntry."Posting Date" := UpdateDate_gDate;
                                        TCSEntry.Modify;
                                    end;
