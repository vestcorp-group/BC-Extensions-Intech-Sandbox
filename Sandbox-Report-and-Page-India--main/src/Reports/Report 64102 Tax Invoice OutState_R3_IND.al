report 64102 "Tax Invoice OutState_R3_IND"//T12370-Full Comment
{
    UsageCategory = Administration;
    ApplicationArea = all;
    DefaultLayout = RDLC;
    RDLCLayout = './Layouts/TAX Invoice_Outside State_R3_IND.rdl';
    Caption = 'TAX Invoice_Outside State_R3_IND';
    ...
    dataset
    {
        dataitem("Sales Invoice Header"; "Sales Invoice Header")
        {
            ...
            dataitem("Sales Invoice Line"; "Sales Invoice Line")
            {
                ...
                column(TtIGST_gDec; TtIGST_gDec) { }
                ...
                begin
                    ...
                    if MergeChargeItem_gBln then begin
                        ...
                        // Sl_chargeItem_gRec.Reset();
                        // Sl_chargeItem_gRec.SetRange("Document No.", "Sales Invoice Header"."No.");
                        // Sl_chargeItem_gRec.SetFilter(Quantity, '<>%1', 0);
                        // Sl_chargeItem_gRec.setrange(Type, Sl_chargeItem_gRec.Type::"Charge (Item)");
                        // if Sl_chargeItem_gRec.FindSet() then
                        //     repeat
                    ...
                    TotalValue := "Sales Invoice Line"."Quantity (Base)" * NewUnitPrice_gDec + (("Sales Invoice Line"."Quantity (Base)" * NewUnitPrice_gDec) * ITPer / 100);
                    TotalValue1 += "Sales Invoice Line"."Quantity (Base)" * NewUnitPrice_gDec + (("Sales Invoice Line"."Quantity (Base)" * NewUnitPrice_gDec) * ITPer / 100);
            ...
            begin
                ...
                //04-04-2025 Commenting as it is not in use and instead we are using seprate texts for Ship to above.
        ...
        layout
        {
            area(Content)
            {
                group(Options)
                {
                    ...
                    //YH++
                    field(MergeChargeItem; MergeChargeItem_gBln)
                    {
                        ApplicationArea = all;
                        Caption = 'Merge Charge Item';
                    }
                    //YH--
    ...
    begin
        ...
        if spacePosition > 1 then
            ...
            CompanyFirstWord := CompanyInformation.Name;
    ...
    var
        ...
        // MergeChargeItem: Boolean; //YH
        Sl_chargeItem_gRec: Record "Sales invoice Line";//YH
        SGSTIC: Decimal;
        CGSTIC: Decimal;
        IGSTIC: Decimal;
        ...
        AmtinWord_GTxtGST: array[2] of Text[100];
