...
report 64103 "Tax Invoice_R3_UAE"
{
    DefaultLayout = RDLC;
    RDLCLayout = './Layouts/Tax Invoice_R3_UAE.rdl';
    Caption = 'Tax Invoice_R3_UAE';
    ...
    dataset
    {
        dataitem("Sales Invoice Header"; "Sales Invoice Header")
        {
            ...
            dataitem("Sales Invoice Line"; "Sales Invoice Line")
            {
                ...
                column(VatAmt; VATAmount_gDec)// SalesLineVatBaseAmount * "VAT %" / 100)
                {
                }
                ...
                column(TotalIncAmt; TotalIncAmt) { }
                ...
                begin
                    ...
                    if PostedCustomInvoiceG then begin
                        ...
                        // else
                        SalesLineAmount := "Sales Invoice Line"."Quantity (Base)" * NewUnitPrice_gDec;
                    ...
                    //     if PostedCustomInvoiceG then begin
                    //         NewLineAmtExclVAT_gDec := "Sales Invoice Line"."Quantity (Base)" * "Sales Invoice Line"."Customer Requested Unit Price" + FindLineAmountDiff_lFnc();
                    //     end else begin
                    //         NewLineAmtExclVAT_gDec := "Sales Invoice Line"."Quantity (Base)" * "Sales Invoice Line"."Unit Price Base UOM 2" + FindLineAmountUOMDiff_lFnc();
                    //     end;
                            ...
                            repeat
                                ...
                                if ("Unit Price" = SalesLineL."Unit Price") and ("Unit of Measure Code" = SalesLineL."Unit of Measure Code") then begin
                                    if PostedCustomInvoiceG then begin
                                        ...
                                        // if MergeChargeItem_gBln then
                                        //     SalesLineVatBaseAmount += Amount + FindAmountDiff_lFnc()//T52816 code commented
                                        // else
                    ...
                    if TempBSONumber <> '' then begin
                        ...
                        TotalAmountSum_gDec := ("Sales Invoice Line"."Quantity (Base)" * NewUnitPrice_gDec) + VATAmount_gDec
                    ...
                    TotalIncAmt += Round(("Sales Invoice Line"."Quantity (Base)" * NewUnitPrice_gDec) + (("Sales Invoice Line"."Quantity (Base)" * NewUnitPrice_gDec) * "Sales Invoice Line"."VAT %" / 100), 0.01, '=');
                    ...
                    if CurrencyCode_gCod = 'INR' then begin
                        Check_LRep.FormatNoText(AmtinWord_GTxt, TotalIncAmt, CurrencyCode_gCod);
                    end else begin
                        Check_USA_lRpt.FormatNoText_USversion(AmtinWord_GTxt, TotalIncAmt, CurrencyCode_gCod);
                    end;
        ...
        layout
        {
            area(Content)
            {
                group(Options)
                {
                    ...
                    //     ApplicationArea = All;
                    // }
                    // field("Commercial invoice"; PrintCaption_gbool)
                    // {
                    //     ApplicationArea = All;
                    // }
    ...
    //     if SL_lRec.FindSet() then begin
    //         repeat
    //             TotalFreight_lDec += SL_lRec."Quantity (Base)" * SL_lRec."Unit Price Base UOM 2";
    //         until SL_lRec.Next() = 0;
    //     end;
        ...
        if SL_lRec.FindSet() then begin
            repeat
                if TempGSTPer_lDec <> 0 then begin
                    if TempGSTPer_lDec <> SL_lRec."VAT %" then
                        ItemGST_lBln := true;
                    // else
                    //     ItemVAT_lBln := false;
                end;
                TempGSTPer_lDec := SL_lRec."VAT %";
            until SL_lRec.Next() = 0;
        end;
    ...
    var
        ...
        CustAddr_Arr: array[9] of Text[250]; //Hypercare 26-02-2025 100-250
        CustAddrShipto_Arr: array[8] of Text[250];//Hypercare 26-02-2025 100-250
