report 54784 "Blanket Sales Order New"//T12370-Full Comment
{
    DefaultLayout = RDLC;
    RDLCLayout = './Layouts/Layout_80003_BlanketSalesOrderReport.rdl';
    Caption = 'Proforma/BSO-CPI';
    ...
    dataset
    {
        dataitem("Sales Header"; "Sales Header")
        {
            ...
            column(companylogo; CompanyInformation.Logo) { }//AS-25032025
  ...
  */
            ...
            dataitem("Sales Line"; "Sales Line")
            {
                ...
                column(UnitPrice; NewUnitPrice_gDec) { } //"Unit Price Base UOM 2"
                ...
                { }
                column(AmountExclGSTSum; AmountExclGSTSum)
                { }
                ...
                begin
                    ...
                    If Item_LRec.GET("No.") THEN BEGIN
                        ...
                        // end;
                        // **end by bayas
                        if "Line Generic Name" <> '' then begin
                            if PrintLineGenric_gbln then
                                SearchDesc := "Line Generic Name"
                            else
                                SearchDesc := '';
                        end;
                        ...
                        if "Unit Price Base UOM 2" = 0 then begin
                            ...
                            //     "Unit Price Base UOM 2" := ("Unit Price" + FindUnitPriceDiff_lFnc())
                            // else
                            "Unit Price Base UOM 2" := "Unit Price";

                            "Base UOM 2" := "Unit of Measure Code";
                            // "Quantity (Base)" := Quantity;//AS
                        end;
                        // if MergeChargeItem_gBln then
                        //     AmountExclGST := Quantity * ("Unit Price" + FindUnitPriceDiff_lFnc())
                        // else
                        // AmountExclGST := Quantity * "Unit Price";// T52416 line commented
                        AmountExclGST := "Quantity (Base)" * NewUnitPrice_gDec;
                    ...
                    End
                        ...
                        if "Sales Line".Type = "Sales Line".Type::"G/L Account" then begin
                            ...
                            //     AmountExclGST := Quantity * ("Unit Price" + FindUnitPriceDiff_lFnc())
                            // else
                            // AmountExclGST := Quantity * "Unit Price"; //T52416 code commented
                            AmountExclGST := "Quantity (Base)" * NewUnitPrice_gDec;
                    ...
                    if MergeChargeItem_gBln then begin
                        // AmountExclGST := AmountExclGST + FindAmountDiff_lFnc();
                        // NewUnitPrice_gDec := AmountExclGST / "Sales Line".Quantity;
                        AmountExclGSTSum += AmountExclGST;
                        GSTAmount := GSTAmount + FindGSTDifference_lFnc();
                        AmountInclGst := AmountExclGST + GSTAmount;
                        IGSTamount := GSTAmount;
                        CGSTamount := GSTAmount / 2;
                        SGSTAmount := GSTAmount / 2;
                    end else begin
                        // NewUnitPrice_gDec := "Unit Price Base UOM 2";// "Unit Price";
                    end;
            ...
            var
                ...
                CustomerMgt_lCdu: Codeunit "Customer Mgt.";
            begin
                //ReportHdr_Txt := '';
                ...
                CustomerMgt_lCdu.CalculateShipBillToOptions(SalesShiptoOption, SalesBillToOption, SalesHeader_lTemp);
        ...
        layout
        {
            area(Content)
            {
                group(Options)
                {
                    ...
                    field(PrintLineGenric_gbln; PrintLineGenric_gbln)
                    {
                        ApplicationArea = all;
                        Caption = 'Print Line Generic';
                    }
    ...
    //     if SL_lRec.FindSet() then begin
    //         repeat
    //             TotalFreight_lDec += SL_lRec.Quantity * SL_lRec."Unit Price";
    //         until SL_lRec.Next() = 0;
    //     end;
    ...
    //     if SL_lRec.FindSet() then begin
    //         repeat
    //             TotalItemAmount_lDec += SL_lRec.Quantity * SL_lRec."Unit Price";
    //         until SL_lRec.Next() = 0;
    //     end;

    //     if TotalItemAmount_lDec <> 0 then
    //         AmountDiff_lDec := Round((TotalFreight_lDec * "Sales Line".Quantity * "Sales Line"."Unit Price") / TotalItemAmount_lDec, 0.01, '=');

    //     if "Sales Line".Quantity <> 0 then
    //         UnitPriceDiff_lDec := Round((AmountDiff_lDec / "Sales Line".Quantity), 0.01, '=');

    //     exit(UnitPriceDiff_lDec);
    // end;
    ...
    begin
        ...
        if SL_lRec.FindSet() then begin
            repeat
                ...
                if SL_lRec."GST Group Code" <> '' then
                    CalculateLineGST("Sales Header", SL_lRec);
                TotalofFreight_lDec += IGSTamount + CGSTamount + SGSTAmount;
            until SL_lRec.Next() = 0;
        end;
        ...
        if SL_lRec.FindSet() then begin
            repeat
                ...
                if SL_lRec."GST Group Code" <> '' then
                    CalculateLineGST("Sales Header", SL_lRec);
                TotalofItem_lDec += IGSTamount + CGSTamount + SGSTAmount;
            until SL_lRec.Next() = 0;
        end;

        if "Sales Line"."GST Group Code" <> '' then
            CalculateLineGST("Sales Header", "Sales Line");
        CurrLineGST_lDec := IGSTamount + CGSTamount + SGSTAmount;
    ...
    //             if SL_lRec."GST Group Code" <> '' then
    //                 CalculateLineGST("Sales Header", SL_lRec);
    //             GSTPer_lDec := CGSTPct + SGSTPct + IGSTPct;
    ...
    //             if SL_lRec."GST Group Code" <> '' then
    //                 CalculateLineGST("Sales Header", SL_lRec);
    //             TempGSTPer_lDec := CGSTPct + SGSTPct + IGSTPct;
    //         until SL_lRec.Next() = 0;
    //     end;
    ...
    var
        ...
        CustAddr_Arr: array[10] of Text[150];
