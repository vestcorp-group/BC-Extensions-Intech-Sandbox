report 53017 "Sales Quotation_Local_IND"
{
    DefaultLayout = RDLC;
    RDLCLayout = 'Sales Print Layout/RDL/Sales Quotation_Local_IND.rdl';
    ...
    dataset
    {
        dataitem("Sales Header"; "Sales Header")
        {
            ...
            dataitem("Sales Line"; "Sales Line")
            {
                ...
                column(AmountIncludingVAT; SalesLineAmountincVat + GSTAmount)
                {
                    // IncludeCaption = true;
                }
                // column(TotalAmt; TotalAmt) { }
                ...
                begin
                    ...
                    if SalesLineL.FindSet() then
                        repeat
                            if CustomInvoiceG then
                                AmountExclGST := SalesLineL."Quantity (Base)" * SalesLineL."Customer Requested Unit Price"
                            else
                                AmountExclGST := SalesLineL.Quantity * SalesLineL."Unit Price";

                            if SalesLineL."GST Group Code" <> '' then
                                CalculateLineGST("Sales Header", SalesLineL);

                            GSTAmt := IGSTamount + CGSTamount + SGSTAmount;
                            GSTPer := Format(CGSTPct + SGSTPct + IGSTPct) + '%';
                            AmountInclGST := AmountExclGST + GSTAmt;

                            TotalAmt += AmountInclGST;
                        until SalesLineL.Next() = 0;

                    // Convert TotalAmt to words after accumulating all lines
                    Check_LRep.InitTextVariable();
                    Check_LRep.FormatNoText(AmtinWord_GTxt, Round(TotalAmt, 0.01), "Sales Header"."Currency Code");

                    // Combine the amount in words into a single String variable
                    String := AmtinWord_GTxt[1] + ' ' + AmtinWord_GTxt[2];
