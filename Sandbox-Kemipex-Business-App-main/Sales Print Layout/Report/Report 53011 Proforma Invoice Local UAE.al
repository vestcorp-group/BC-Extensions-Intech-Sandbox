report 53011 "Proforma Invoice_Local UAE"
{
    DefaultLayout = RDLC;
    RDLCLayout = 'Sales Print Layout/RDL/Proforma Invoice_Local.rdl';
    ...
    dataset
    {
        dataitem("Sales Header"; "Sales Header")
        {
            ...
            column(PostingDate; Format("Order Date", 0, '<Day,2>-<Month Text>-<Year4>')) { }
            column(BilltoName; "Bill-to Name") { }
            /*Column(ReportHdr_Txt; ReportHdr_Txt) { }*/
            column(CompName; CompanyInformation.Name) { }
            column(CompanyFirstWord; CompanyFirstWord) { }
            column(GST; CompanyInformation."Enable GST Caption") { }
            column(CompLogo; CompanyInformation.Picture) { }
            column(CompInfo_gRec_Logo; CompanyInformation.Logo) { }
            // column(AmtinWord_GTxt; String + ' ' + CurrencyDescription) { }//AS-Commented 06032025
            ...
            column(CustomInvoiceG; CustomInvoiceG) { }
            column(Remark; '')
            {
            }
            ...
            column(PortofLoading_SalesHeader; ExitPtDesc)
            {
                /*IncludeCaption = true;*/
            }
            ...
            dataitem("Sales Line"; "Sales Line")
            {
                ...
                column(UnitPrice; NewUnitPrice_gDec) { } //Customeunitprice
                ...
                column(VatAmt_gDec; SalesLineVatBaseAmount * "VAT %" / 100) { }
                ...
                begin
                    ...
                    if CustomInvoiceG then begin
                        ...
                        SL_lRec.Reset();
                        SL_lRec.SetRange("Document No.", "Sales Header"."No.");
                        SL_lRec.SetRange(Type, "Sales Line".Type::"Charge (Item)");
                        if SL_lRec.FindSet() then begin
                            repeat
                    ...
                    SL_lRec.Reset();
                    SL_lRec.SetRange("Document No.", "Sales Header"."No.");
                    SL_lRec.SetRange(Type, "Sales Line".Type::Item);
                    if SL_lRec.FindSet() then begin
                        repeat
                    ...
                    if MergeChargeItem_gBln then begin
                        ...
                        if CustomInvoiceG then begin
                            NewUnitPrice_gDec := "Sales Line"."Customer Requested Unit Price" + TotalFreight_gDec;
                        end else begin
                            NewUnitPrice_gDec := "Sales Line"."Unit Price Base UOM 2" + TotalFreight_gDec;
                        end;
                    end else begin
                        if CustomInvoiceG then
                            NewUnitPrice_gDec := "Sales Line"."Customer Requested Unit Price"
                        else
                            NewUnitPrice_gDec := "Sales Line"."Unit Price Base UOM 2";
                    end;
                                ...
                                repeat
                                    if CustomInvoiceG then begin
                                        if ("Customer Requested Unit Price" = SalesLineL."Customer Requested Unit Price") and ("Unit of Measure Code" = SalesLineL."Unit of Measure Code") then begin
                                            ...
                                            //     SalesLineAmount += SalesLineL."Quantity (Base)" * "Customer Requested Unit Price" + FindLineAmountDiff_lFnc()
                                            // else
                                            // SalesLineAmount += SalesLineL."Quantity (Base)" * "Customer Requested Unit Price";
                                            SalesLineAmount += SalesLineL."Quantity (Base)" * NewUnitPrice_gDec;
                    ...
                    if not SalesLineMerge then begin
                        ...
                        if SalesLineL.FindSet() then
                            repeat
                                ...
                                // if CustomInvoiceG then begin
                                //     if ("Customer Requested Unit Price" = SalesLineL."Customer Requested Unit Price") and ("Unit of Measure Code" = SalesLineL."Unit of Measure Code") then begin
                                //         SalesLineAmount := SalesLineL."Quantity (Base)" * SalesLineL."Customer Requested Unit Price";
                                ...
                                //         "Net Weight" += SalesLineL."Net Weight";
                                //         "Gross Weight" += SalesLineL."Gross Weight";
                                //         VatAmt_gDec += SalesLineAmount * SalesLineL."VAT %" / 100;
                                //     end;
                                // end
                                // else
                                if ("Unit Price" = SalesLineL."Unit Price") and ("Unit of Measure Code" = SalesLineL."Unit of Measure Code") then begin
                                    ...
                                    "Gross Weight" += SalesLineL."Gross Weight";
                                    "Net Weight" += SalesLineL."Net Weight";
                                    VatAmt_gDec += SalesLineVatBaseAmount * "VAT %" / 100;
                    ...
                    if CustomInvoiceG then begin
                        ...
                        // NewUnitPrice_gDec := "Sales Line"."Customer Requested Unit Price";
                        // if MergeChargeItem_gBln then
                        //     SalesLineAmount := "Sales Line"."Quantity (Base)" * "Sales Line"."Customer Requested Unit Price" + FindLineAmountDiff_lFnc()
                        // else
                        // SalesLineAmount := "Sales Line"."Quantity (Base)" * "Sales Line"."Customer Requested Unit Price";
                        SalesLineAmount := "Sales Line"."Quantity (Base)" * NewUnitPrice_gDec;
                        ...
                        Customeunitprice := "Sales Line"."Unit Price Base UOM 2"; //PackingListExtChange
                                                                                  // NewUnitPrice_gDec := "Sales Line"."Unit Price Base UOM 2";
                                                                                  // if MergeChargeItem_gBln then
                                                                                  //     SalesLineAmount := "Sales Line"."Line Amount" + FindLineAmountDiff_lFnc()
                                                                                  // else

                        SalesLineAmount := "Sales Line"."Quantity (Base)" * NewUnitPrice_gDec;                                                        // SalesLineAmount := "Sales Line"."Line Amount";
                                                                                                                                                      // SalesLineAmount := NewUnitPrice_gDec;//T52416 commented for a while.

                        // if MergeChargeItem_gBln then
                        //     SalesLineAmountincVat := "Sales Line"."Amount Including VAT" + FindAmountInclVATDiff_lFnc()
                        // else

                        // SalesLineAmountincVat := "Sales Line"."Amount Including VAT"; //T52416 commented
                        SalesLineAmountincVat := (SalesLineAmount * "VAT %" / 100) + SalesLineAmount;

                        // if MergeChargeItem_gBln then
                        //     SalesLineVatBaseAmount := "Sales Line"."VAT Base Amount" + FindVATBaseAmountDiff_lFnc()
                        // else
                    ...
                    // if MergeChargeItem_gBln then begin
                    //     NewUnitPrice_gDec := SalesLineAmount / "Quantity (Base)";
                    // end;
                    //SD::GK
                    //UK::03062020<<
                end;
            ...
            begin
                ...
                //T52416-NS
                if MergeChargeItem_gBln then
                    if CheckVATPercentage_lFnc() then
                ...
                if "Sales Header"."Exit Point" <> '' then begin
                    if ExitpointRec.Get("Sales Header"."Exit Point") then
                        ExitPtDesc := ExitpointRec.Description;
                end;
                //SD::GK
    ...
    local procedure CheckVATPercentage_lFnc(): Boolean
    var
        SL_lRec: Record "Sales Line";
        VATPer_lDec: Decimal;
        ItemVAT_lBln: Boolean;
        ChargesVAT_lBln: Boolean;
    begin
        SL_lRec.Reset();
        ...
        if SL_lRec.FindSet() then begin
            repeat
                if VATPer_lDec <> 0 then begin
                    if SL_lRec."VAT %" <> VATPer_lDec then
                        ItemVAT_lBln := true;
                    // else
                    //     ItemVAT_lBln := false;
                end;
                VATPer_lDec := SL_lRec."VAT %";
            until SL_lRec.Next() = 0;
        end;
        ...
        //         if VATPer_lDec <> 0 then begin
        //             if SL_lRec."VAT %" <> VATPer_lDec then
        //                 ChargesVAT_lBln := true
        //             else
        //                 ChargesVAT_lBln := false;
        //         end;
        //         VATPer_lDec := SL_lRec."VAT %";
        //     until SL_lRec.Next() = 0;
        // end;

        if ItemVAT_lBln then
            exit(true)
        else
            exit(false);
    end;
    ...
    //     if SL_lRec.FindSet() then begin
    //         repeat
    //             TotalFreight_lDec += SL_lRec."Quantity (Base)" * SL_lRec."Customer Requested Unit Price";
    //         until SL_lRec.Next() = 0;
    //     end;
    ...
    //         LineAmountDiff_lDec := Round((TotalFreight_lDec * "Sales Line"."Quantity (Base)" * "Sales Line"."Customer Requested Unit Price") / TotalItemAmount_lDec, 0.01, '=');
    ...
    //     if TotalItemAmount_lDec <> 0 then
    //         LineAmountDiff_lDec := Round((TotalFreight_lDec * "Sales Line"."Amount Including VAT") / TotalItemAmount_lDec, 0.01, '=');
    ...
    //     if SL_lRec.FindSet() then begin
    //         repeat
    //             TotalFreight_lDec += SL_lRec.Amount;
    //         until SL_lRec.Next() = 0;
    //     end;
    ...
    //     if SL_lRec.FindSet() then begin
    //         repeat
    //             TotalItemAmount_lDec += SL_lRec.Amount;
    //         until SL_lRec.Next() = 0;
    //     end;

    //     if TotalItemAmount_lDec <> 0 then
    //         AmountDiff_lDec := Round((TotalFreight_lDec * "Sales Line".Amount) / TotalItemAmount_lDec, 0.01, '=');
    ...
    //     if TotalItemAmount_lDec <> 0 then
    //         VATBaseAmountDiff_lDec := Round((TotalFreight_lDec * "Sales Line"."VAT Base Amount") / TotalItemAmount_lDec, 0.01, '=');
    ...
    var
        ...
        Condition7: TextConst ENU = 'Notice of claims arising out of damage in transit must be logged by the Buyer directly with the carrier within the';
