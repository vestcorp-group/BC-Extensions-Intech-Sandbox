page 58173 ConsolInventoryNPriceforSales//T12370-Full Comment   //T13413-Full UnComment
{
    PageType = List;
    ApplicationArea = All;
    UsageCategory = Administration;
    Caption = 'Item Price and Consolidated Inventory';
    SourceTableView = sorting("Search Description") order(ascending) where("Inventory Posting Group" = filter('PD'), "Consolidated Inventory" = filter('<>0'), "Sales Blocked" = filter('false'), "Blocked" = filter('false'));
    SourceTable = item;
    Editable = false;
    InsertAllowed = false;
    DeleteAllowed = false;
