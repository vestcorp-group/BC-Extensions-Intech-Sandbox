report 58181 CustomerStatement_2//T12370-Full Comment
{
    Caption = 'Customer Statement 2';
    DefaultLayout = RDLC;
    RDLCLayout = './Reports/KMP_CustomerStatement.rdl';
    ...
    dataset
    {
        ...
        Company)
        {
            ...
            dataitem(Customer; Customer)
            {
                ...
                dataitem("Cust. Ledger Entry"; "Cust. Ledger Entry")
                {
                    ...
                    column(BSO_Date;
                    Format(BSODateG, 0, '<Day,2>/<Month,2>/<year4>'))
                    {
                    }
                    column(SO_No_;
                    SONoG)
                    ...
                    column(SO_Date;
                    Format(SODateG, 0, '<Day,2>/<Month,2>/<year4>'))
                    {
                    }
                    column(Invoice_Date;
                    Format(InvoiceDateG, 0, '<Day,2>/<Month,2>/<year4>'))
                    {
                    }
                    column(Document_No_;
                    "Document No.")
                    ...
                    column(Outstanding_Balance__LCY_;
                    "Remaining Amt. (LCY)")
                    {
                    }
                    column(Due_Days;
                    ...
                    begin
                        ...
                        Clear(SODateG);
                        Clear(BSODateG);
                        case ReportFilterG of // ReportFilterG::"Outstanding":
                        ...
                        GLSetupG.Get();
                        if "Currency Code" = '' then "Currency Code" := GLSetupG."LCY Code";
                        if (CurrencyCodeG > '') and (CurrencyCodeG <> "Currency Code") then CurrReport.Skip();
                        if (SalesInvoiceG > '') and (SalesInvoiceG <> "Document No.") then CurrReport.Skip();
                        ...
                        if SalesInvLnL.FindFirst() then begin
                            ...
                            if (SalesOrderG > '') and (SalesOrderG <> SONoG) then CurrReport.Skip();
                            if SalesHdrL.Get(SalesHdrL."Document Type"::Order, SalesInvLnL."Order No.") then
                                ...
                                if SalesHdrArchiveL.FindFirst() then SODateG := SalesHdrArchiveL."Document Date";
                            end;
                            ...
                            if SalesHdrL.Get(SalesHdrL."Document Type"::"Blanket Order", SalesInvLnL."Blanket Order No.") then
                                ...
                                SalesHdrArchiveL.SetRange("No.", SalesInvLnL."Order No.");
                                if SalesHdrArchiveL.FindFirst() then BSODateG := SalesHdrArchiveL."Document Date";
                            end;
                        end;
                ...
                begin
                    ...
                    if SalesPersonG > '' then SetFilter("Salesperson Code", SalesPersonG);
                    if not ShowIntercompanyCustG then SetFilter("IC Partner Code", '%1', '');
                end;
                ...
                begin
                    ...
                    if Alternat_customer_rec.Get("No.") then
                        "Search Name" := Alternat_customer_rec."Alternate Short Name";
                    ...
                    if SalesInvoiceG > '' then
                        if not SalesInvHdrL.Get(SalesInvoiceG) then
                            ...
                            if "No." <> SalesInvHdrL."Bill-to Customer No." then CurrReport.Skip();
                    if "Salesperson Code" > '' then begin
                        ...
                        if SalespersonTeamsG > '' then begin
                            ...
                            if not TeamSalespersonL.FindFirst() then CurrReport.Skip();
                        end;
        ...
        Integer)
        {
            column(TotalInvoiceCaption;
            'Total Invoice')
            {
            }
            column(Total_Currency_Code;
            TempCurrencyG.Code)
            {
            }
            column(Total_Invoice_Value;
            TempCurrencyG."Customer Balance")
            {
            }
            trigger OnPreDataItem()
            var
                myInt: Integer;
            begin
                if not TempCurrencyG.FindSet() then CurrReport.Break();
    ...
    var
        ...
        Alternat_customer_rec: Record "Customer Alternet Address";
        CustomerName: Text[2000];
}
