report 53008 "7_Purchase Order R3 UAE"//Don't Use this report --------currently not use from multi destination
{
    DefaultLayout = RDLC;
    RDLCLayout = 'Reports/7_Purchase Order R3 UAE.rdl';
    // ApplicationArea = All;
    ...
    dataset
    {
        dataitem("Purchase Header"; "Purchase Header")
        {
            ...
            { }
            column(Inspection_gTxt; Inspection_gTxt)
            { }
            ...
            "Purchase Line")
            {
                ...
                begin
                    ...
                    Clear(Area_lRec);
                    POD_gTxt := '';
                    if "Purchase Line"."Area" <> '' then begin
                        Area_lRec.Get("Purchase Line"."Area");
                        POD_gTxt := Area_lRec.Text;
                    end;

                    Clear(EntryExitPoint_lRec);
                    POL_gTxt := '';
                    if "Purchase Line"."Entry Point" <> '' then begin
                        EntryExitPoint_lRec.Get("Purchase Line"."Entry Point");
            ...
            begin
                ...
                if Area_lRec.Get("Purchase Header"."Area") then;
                Clear(VendAddr);
