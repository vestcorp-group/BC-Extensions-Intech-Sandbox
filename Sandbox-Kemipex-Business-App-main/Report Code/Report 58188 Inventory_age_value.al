report 58188 Inventory_age_value//T12370-Full Comment T12946-Code Uncommented
{
    UsageCategory = ReportsAndAnalysis;
    ApplicationArea = All;
    DefaultLayout = RDLC;
    RDLCLayout = 'Reports/Inventory Age Composition and Valuation.rdl';
    Caption = 'Inventory Age Composition and Valuation';
    ...
    dataset
    {
        dataitem(Item; Item)
        {
            ...
            begin
                ...
                if ILE.FindSet() then
                    repeat
                        if (ile."Posting Date" > bracket30D) then begin
                        ...
                        if (ile."Posting Date" > bracket90D) and (ile."Posting Date" <= bracket30D) then begin
                            itemapplication.Reset();
                            itemapplication.SetRange("Inbound Item Entry No.", ILE."Entry No.");
                            itemapplication.SetFilter("Posting Date", '..%1', AsonDate);
                            itemapplication.CalcSums(Quantity);
                        ...
                        if (ile."Posting Date" > bracket180D) and (ile."Posting Date" <= Bracket90D) then begin
                            itemapplication.Reset();
                            itemapplication.SetRange("Inbound Item Entry No.", ILE."Entry No.");
                            itemapplication.SetFilter("Posting Date", '..%1', AsonDate);
                            itemapplication.CalcSums(Quantity);
                        ...
                        if (ile."Posting Date" > bracket360D) and (ile."Posting Date" <= bracket180D) then begin
                            itemapplication.Reset();
                            itemapplication.SetRange("Inbound Item Entry No.", ILE."Entry No.");
                            itemapplication.SetFilter("Posting Date", '..%1', AsonDate);
                            itemapplication.CalcSums(Quantity);
                        ...
                        if ile."Posting Date" < bracket720D then begin
                            itemapplication.Reset();
                            itemapplication.SetRange("Inbound Item Entry No.", ILE."Entry No.");
                            itemapplication.SetFilter("Posting Date", '..%1', AsonDate);
                            itemapplication.CalcSums(Quantity);
