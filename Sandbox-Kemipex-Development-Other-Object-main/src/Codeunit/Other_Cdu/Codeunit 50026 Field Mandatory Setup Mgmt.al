codeunit 50026 "Field Mandatory Setup Mgmt."
{
    Description = 'T12141';

    var
        GlobalWorkflowTemplate: Record "Field Mand. Templ. Setup";
        RecordConditionTxt: Label 'Record Conditions - %1', Comment = '%1 = Table Caption';

    procedure OpenDynamicRequestPage(var FieldMandTempSetup: Record "Field Mand. Templ. Setup")
    var
        RecordRef: RecordRef;
        RequestFilters: Text;
        ReturnFilters: Text;
    ...
    begin
        GlobalWorkflowTemplate := FieldMandTempSetup;
        RecordRef.GetTable(FieldMandTempSetup);
        RequestFilters := GetRequestFilters(RecordRef);
        UserClickedOK := RunDynamicRequestPage(ReturnFilters, RequestFilters, RecordRef);

        if UserClickedOK then
            SetTablesCondition(FieldMandTempSetup, ReturnFilters);
    end;

    procedure SetTablesCondition(var PostingNoSeries: Record "Field Mand. Templ. Setup"; Filters: Text)
    var
    ...
    procedure GetRequestFilters(Recordref: RecordRef) Filters: Text
    var
        ConditionInStream: InStream;
    begin
        GlobalWorkflowTemplate.CalcFields(Condition);
        GlobalWorkflowTemplate.Condition.CreateInStream(ConditionInStream);
    ...
    begin
        if not BuildDynamicRequestPage(FilterPageBuilder, '', GlobalWorkflowTemplate."Table ID") then
            exit(false);

        if (Filters <> '') and not (SetViewOnDynamicRequestPage(FilterPageBuilder, Filters, '', GlobalWorkflowTemplate."Table ID")) then
            exit(false);

        FilterPageBuilder.PageCaption := StrSubstNo(RecordConditionTxt, GetTableCaption(GlobalWorkflowTemplate."Table Id"));

        Commit();
        if not FilterPageBuilder.RunModal() then
            exit(false);

        ReturnFilters := FilterPageBuilder.GetView(GetTableCaption(GlobalWorkflowTemplate."Table Id"), false);
        exit(true);
    end;
