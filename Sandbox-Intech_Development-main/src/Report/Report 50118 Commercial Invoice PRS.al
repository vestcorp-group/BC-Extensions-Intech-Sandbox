//T48657
report 50118 "Commercial Invoice PRS"
{
    DefaultLayout = RDLC;
    RDLCLayout = './Layouts/50118 CommercialInvoice.rdl';
    ...
    dataset
    {
        dataitem("Purch. Cr. Memo Hdr."; "Purch. Cr. Memo Hdr.")
        {
            ...
            { }
            column(BilltoName_SalesInvoiceHeader; "Pay-To Name")
            { }
            ...
            column(Bill_to_City; "Pay-to City")
            {
            }
            ...
            // column(PaymentTerms; "Purch. Cr. Memo Hdr."."Payment Terms Code")
            // {
            // }
            ...
            { }
            column(SNo; SNo) { }
            column(Order_Date; FORMAT("Document Date", 0, '<Day,2>/<Month,2>/<Year4>'))
            {
            }
            ...
            //AW09032020>>
            column(PI_Validity_Date; Format("Due Date", 0, '<Day,2>/<Month,2>/<Year4>'))
            {
            }
            ...
            dataitem("Purch. Cr. Memo Line"; "Purch. Cr. Memo Line")
            {
                ...
                begin
                    //if DoNotShowGL then
                    //  SetFilter(Type, '<>%1', "Purch. Cr. Memo Line".Type::"G/L Account");
                ...
                var
                    ...
                    SalesLineL: Record "Purch. Cr. Memo Line";
                    SalesHeaderRec: Record "Purchase Header";
                    ...
                    SalesInvHeaderRec: Record "Purch. Cr. Memo Hdr.";
                ...
                begin
                    ...
                    If "Document No." <> '' then
                        if SalesInvHeaderRec.get("Document No.") then;


                    // if "VAT Prod. Posting Group"
                    //GK
                    if PostedCustomInvoiceG then begin
                        Customeunitprice := "Purch. Cr. Memo Line"."Unit Cost (LCY)";
                        SalesLineAmount := "Purch. Cr. Memo Line"."Quantity (Base)" * "Purch. Cr. Memo Line"."Unit Cost (LCY)";
                        SalesLineAmountincVat := (SalesLineAmount * "VAT %" / 100) + SalesLineAmount;
                        SalesLineVatBaseAmount := SalesLineAmount;

                    end
                    else begin
                        Customeunitprice := "Purch. Cr. Memo Line"."Unit Cost (LCY)"; //PackingListExtChange
                        SalesLineAmount := "Purch. Cr. Memo Line".Amount;
                        SalesLineAmountincVat := "Purch. Cr. Memo Line"."Amount Including VAT";
                        SalesLineVatBaseAmount := "Purch. Cr. Memo Line"."VAT Base Amount";
                    end;
                    //GK

                    // ShowVat in ""
                    // if "Purch. Cr. Memo Line"."VAT Prod. Posting Group"=''
                    //psp
                    ...
                    SalesLineL.SetRange("Unit Cost (LCY)", "Unit Cost (LCY)");
                    ...
                    if SalesLineL.FindSet() then
                        repeat
                            ...
                            if ("Unit Cost (LCY)" = SalesLineL."Unit Cost (LCY)") and ("Unit of Measure Code" = SalesLineL."Unit of Measure Code") then begin
                                if PostedCustomInvoiceG then begin
                                    Clear(SalesLineAmt);
                                    Clear(SalesLineAmtIncVat);
                                    Clear(SalesLineVatBaseAmt);
                                    SalesLineAmt := SalesLineL."Quantity (Base)" * "Unit Cost (LCY)";
                                    Amount += SalesLineAmt;
                    ...
                    if ("Purch. Cr. Memo Line".Type = "Purch. Cr. Memo Line".Type::Item) AND ("Quantity (Base)" = 0) then
                        CurrReport.Skip();

                    if ("Purch. Cr. Memo Line".Type = "Purch. Cr. Memo Line".Type::"G/L Account") AND (PostedDoNotShowGL) then
                        CurrReport.Skip();
                    ...
                    If Item_LRec.GET("No.") THEN BEGIN
                        ...
                        if SalesInvHeaderRec."Pay-to Country/Region Code" = 'IND' then begin // These fiels only for Indian Customers
                    ...
                    // SalesLineL.SetRange("Document No.", "Purch. Cr. Memo Line"."Document No.");
                    // SalesLineL.SetRange("Line No.", "Purch. Cr. Memo Line"."Line No.");
                    ...
                    //     //UK::08062020>>
                    // end else begin
                    //     HSNCode := "Purch. Cr. Memo Line".HSNCode;
                    //     Origitext := "Purch. Cr. Memo Line".CountryOfOrigin;
                    // end;
                    //UK::08062020<<
                    //UK::04062020>>
                    //code commented due to reason
                    // SalesHeaderRec.Reset();
                    // SalesHeaderRec.SetRange("No.", "Purch. Cr. Memo Line"."Blanket Order No.");
                    // if SalesHeaderRec.FindFirst() then begin     
                    //UK::24062020>>
                    if "Purch. Cr. Memo Line"."Blanket Order No." <> '' then begin
                        // if PostedCustomInvoiceG then
                        //     PINONew := "Purch. Cr. Memo Line"."Blanket Order No." + '-A'
                        // else
                        PINONew := "Purch. Cr. Memo Line"."Blanket Order No.";
                        SalesHeader.Reset();
                        SalesHeader.SetRange("Document Type", SalesHeader."Document Type"::"Blanket Order");
                        SalesHeader.SetRange("No.", "Purch. Cr. Memo Line"."Blanket Order No.");
                        if SalesHeader.FindFirst() then
                            ...
                            salesHeaderArchive.SetRange("No.", "Purch. Cr. Memo Line"."Blanket Order No.");
                            if salesHeaderArchive.FindFirst() then
                                PIDateNew := salesHeaderArchive."Order Date";
                        end;
                    end else begin
                        PINONew := "Purch. Cr. Memo Line"."Order No.";
                        SalesHeader.Reset();
                        SalesHeader.SetRange("Document Type", SalesHeader."Document Type"::Order);
                        SalesHeader.SetRange("No.", "Purch. Cr. Memo Line"."Order No.");
                        if SalesHeader.FindFirst() then
                            PIDateNew := SalesHeader."Order Date"
                        else begin
                            salesHeaderArchive.Reset();
                            salesHeaderArchive.SetRange("Document Type", salesHeaderArchive."Document Type"::Order);
                            salesHeaderArchive.SetRange("No.", "Purch. Cr. Memo Line"."Order No.");
                            if salesHeaderArchive.FindFirst() then
                                PIDateNew := salesHeaderArchive."Order Date";
                        end;
                    end;
                    //UK::24062020<<
                    // PIDateNew := "Purch. Cr. Memo Hdr."."Order Date";
                    // end
                    // else begin
                    // PINONew := "Order No.";
                    // PIDateNew := "Purch. Cr. Memo Hdr."."Order Date";
                    // end;
                    //UK::04062020<<
                    // if SalesShipHdr.Get("Purch. Cr. Memo Line"."Shipment No.") then
                    //     SalesOrderNo := SalesShipHdr."Order No."
                    // else
                    SalesOrderNo := "Purch. Cr. Memo Line"."Order No.";

                    //UK::03062020>>
                    // if PostedCustomInvoiceG then begin
                    //     HSNCode := "Purch. Cr. Memo Line".LineHSNCode;
                    //     //UK::24062020>>
                    //     CountryRegRec.Reset();
                    //     if CountryRegRec.Get("Purch. Cr. Memo Line".LineCountryOfOrigin) then
                    //         Origitext := CountryRegRec.Name;
                    // end else begin
                    //     HSNCode := "Purch. Cr. Memo Line".HSNCode;
                    //     CountryRegRec.Reset();
                    //     if CountryRegRec.Get("Purch. Cr. Memo Line".CountryOfOrigin) then
                    //         Origitext := CountryRegRec.Name;
                    // end;
                    //UK::24062020<<
                    //UK::03062020<<
            ...
            dataitem("Sales Remark Archieve"; "Purchase Remark Archieve")
            {
            ...
            trigger OnAfterGetRecord()
            var
                SalesInvoiceLine_LRec: Record "Purch. Cr. Memo Line";
                ...
                SalesShipHdr: Record "Sales Shipment Header";
                Comment_Lrec: Record "Purch. Comment Line";
                TranSpec_rec: Record "Transaction Specification";
                Area_Rec: Record "Area";
                SalesInvoiceLine_L: Record "Purch. Cr. Memo Line";
            ...
            begin
                ...
                ExitPtDesc := '';
                Clear(CustAddr_Arr);
                //FormatAddr.SalesInvBillTo(CustAddr_Arr, "Purch. Cr. Memo Hdr."); //AW-06032020
                //FormatAddr.SalesHeaderBillTo(CustAddr_Arr, "Purch. Cr. Memo Hdr."); //AW-06032020
                GLSetup.Get();
                TranSec_Desc := '';
                ...
                AreaRec.Reset();
                IF AreaRec.Get("Purch. Cr. Memo Hdr."."Area") then
                    AreaDesc := AreaRec.Text;

                if PrintCustomerAltAdd then begin
                    Clear(AreaDesc);
                    IF AreaRec.Get("Purch. Cr. Memo Hdr."."Entry Point") then
                        AreaDesc := AreaRec.Text;
                end;
                IF Area_Rec.Get("Area") And (Area_Rec.Text <> '') then
                    TranSec_Desc := TranSec_Desc + ', ' + AreaDesc;

                if ("Purch. Cr. Memo Hdr."."VAT Bus. Posting Group" = 'C-DZ') OR ("Purch. Cr. Memo Hdr."."VAT Bus. Posting Group" = 'C-LAT-DG') then
                ...
                //PartialShipment
                // if "Purch. Cr. Memo Hdr."."Shipment Term" = "Purch. Cr. Memo Hdr."."Shipment Term"::"Partial Shipment" then begin
                //     Clear(I);
                //     SalesInvoicelineRec.Reset();
                //     SalesInvoicelineRec.SetRange("Document No.", "Purch. Cr. Memo Hdr."."No.");
                ...
                //             else
                //                 ShipmentS := "Purch. Cr. Memo Hdr."."Shipment Count";
                //         end
                //         else
                //             ShipmentS := "Purch. Cr. Memo Hdr."."Shipment Count";
                //     end;
                ...
                // end else
                //     if "Purch. Cr. Memo Hdr."."Shipment Term" = "Purch. Cr. Memo Hdr."."Shipment Term"::"One Shipment" then
                //         ShipmentNo := Format("Purch. Cr. Memo Hdr."."Shipment Term");
                // if ShipmentNo <> '' then
                ...
                if PrintCustomerAltAdd = true then begin
                    if CustomerAltAdd.Get("Sell-to Customer No.") then begin
                        ...
                        if CountryRegionL.Get("Pay-To Country/Region Code") then
                            CustAddr_Arr[6] := CountryRegionL.Name;
                        CustomerRec.Reset();
                        if CustomerRec.Get("Sell-To Customer No.") then
                            if CustomerRec."Phone No." <> '' then
                                ...
                                if "Pay-to Contact No." <> '' then
                                    CustAddr_Arr[7] := 'Tel No.: ' + "Pay-to Contact No.";
                ...
                // if TransactionSpecificationRec.Get("Purch. Cr. Memo Hdr."."Transaction Specification") then begin
                //     if TransactionSpecificationRec."Insurance By" = TransactionSpecificationRec."Insurance By"::Seller then begin
                //         InsurancePolicy := 'Insurance Policy: ' + "Purch. Cr. Memo Hdr."."Insurance Policy No." + ' Provided by ' + Format(TransactionSpecificationRec."Insurance By");
                //     end
                //     else
                ...
                if SalesInvoiceLine_LRec.FindSet(false) then
                    repeat
                        if PostedCustomInvoiceG then begin
                            TotalAmt += Round((salesInvoiceLine_LRec."Quantity (Base)" * salesInvoiceLine_LRec."Unit Cost (LCY)") + ((salesInvoiceLine_LRec."Quantity (Base)" * salesInvoiceLine_LRec."Unit Cost (LCY)") * SalesInvoiceLine_LRec."VAT %" / 100), 0.01, '=');
                            TotalVatAmtAED += Round((salesInvoiceLine_LRec."Quantity (Base)" * salesInvoiceLine_LRec."Unit Cost (LCY)") * SalesInvoiceLine_LRec."VAT %" / 100, 0.01, '=');
                            AmtExcVATLCY += Round(SalesInvoiceLine_LRec."Quantity (Base)" * SalesInvoiceLine_LRec."Unit Cost (LCY)", 0.01, '=');
                        end
                        else begin
                            ...
                            AmtExcVATLCY += SalesInvoiceLine_LRec.Quantity * SalesInvoiceLine_LRec."Unit Cost (LCY)";
                        end;
                ...
                if CustomerRec.Get("Sell-to Customer No.") And (CustomerRec."seller Bank Detail" = false) then begin
                    ...
                    SalesInvoiceLine_LRec.SetRange("Document No.", "Purch. Cr. Memo Hdr."."No.");
                    SalesInvoiceLine_LRec.SetRange(Type, SalesInvoiceLine_LRec.Type::Item);
                    if SalesInvoiceLine_LRec.FindFirst() then begin
                        SalesShipLine.Reset();
                        IF SalesShipLine.Get(SalesInvoiceLine_LRec."Order No.", SalesInvoiceLine_LRec."Line No.") then begin
                            SalesShipHdr.Reset();
                            ...
                            LCNumebr := ''; //PackingListExtChange
                            LCDate := 0D; //PackingListExtChange
                            // LegalizationRequired := ''; //PackingListExtChange
                            // InspectionRequired := ''; //PackingListExtChange
                            //IF BankNo <> '' then
                            //BankNo := "Bank on Invoice 2"; //PackingListExtChange
                            If Bank_LRec.GET(BankNo) then begin
                                BankName := Bank_LRec.Name;
                ...
                PmtTrmRec.Reset();
                IF PmtTrmRec.Get("Purch. Cr. Memo Hdr."."Payment Terms Code") then
                ...
                // PortOfLoding := "Purch. Cr. Memo Hdr.".CountryOfLoading;
                // if PostedCustomInvoiceG then begin
                //     clear(PortOfLoding);
                //     PortOfLoding := "Purch. Cr. Memo Hdr."."Customer Port of Discharge";
                // end;

                ExitPt.Reset();
                IF ExitPt.Get("Purch. Cr. Memo Hdr."."Entry Point") then
                ...
                // IF "Purch. Cr. Memo Hdr."."Inspection Required 2" = true then begin //PackingListExtChange
                //     SerialNo += 1;
                //     SrNo3 := SerialNo;
                // end;
                // IF "Purch. Cr. Memo Hdr."."Legalization Required 2" = true then begin //PackingListExtChange
                //     SerialNo += 1;
                //     SrNo4 := SerialNo;
                // end;

                // if "Purch. Cr. Memo Hdr."."Duty Exemption" = true then begin
                //     SerialNo += 1;
                //     SrNo5 := SerialNo;
                // end;
                ...
                //SalesShipHdr.SetRange("No.", "Purch. Cr. Memo Line"."Shipment No.");
    ...
    var
        ...
        SalesInvoicelineRec: Record "Purch. Cr. Memo Line";
