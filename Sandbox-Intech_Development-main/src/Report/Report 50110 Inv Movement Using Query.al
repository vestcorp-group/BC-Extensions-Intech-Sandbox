report 50110 "Inventory Movement Using Query"
{
    UsageCategory = ReportsAndAnalysis;
    ApplicationArea = All;
    ProcessingOnly = true;
    Description = 'T47531';
    Caption = 'Inventory Movement Report';
    ...
    begin
        ...
        //     InvMovement_gQry.SetFilter(Location_Code, FilterLoc_gTxt)
        // else
        //     InvMovement_gQry.SetFilter(Location_Code, '<>%1', '');

        InvMovement_gQry.SetFilter(Posting_Date, '<=%1', EndDate_gDte);
        Win_gDlg.Open('Fetching Data #1#############\Current #2##############');
        //Win_gDlg.Update(1, 1); //Count);


        if InvMovement_gQry.Open() then begin
            while InvMovement_gQry.Read() do begin
                if InvMovement_gQry.Location_Code <> '' then begin
                    Opening := 0;
                    Purchase := 0;
                    Output := 0;
                    Positive := 0;
                    ...
                    SalesRtrnQty := 0;
                    SalesRtrnAmt := 0;
                    if not ((ItemNo_gCod = InvMovement_gQry.Item_No_) and (InvMovement_gQry.Location_Code = LocCode_gCod) and (Variant_gCod = InvMovement_gQry.Variant_Code)) then begin

                        ItemNo_gCod := InvMovement_gQry.Item_No_;
                        LocCode_gCod := InvMovement_gQry.Location_Code;
                        Variant_gCod := InvMovement_gQry.Variant_Code;

                        ValueEntry_GRec.Reset;
                        ValueEntry_GRec.SetRange("Item No.", InvMovement_gQry.Item_No_);
                        ValueEntry_GRec.SetFilter("Location Code", InvMovement_gQry.Location_Code);
                        ValueEntry_GRec.SetRange("Posting Date", StartDate_gDte, EndDate_gDte);
                        ValueEntry_GRec.SetRange("Variant Code", InvMovement_gQry.Variant_Code);
                        ...
                        ValueEntry_GRec.Reset;
                        ValueEntry_GRec.SetRange("Item No.", InvMovement_gQry.Item_No_);
                        ValueEntry_GRec.SetRange("Location Code", InvMovement_gQry.Location_Code);
                        ValueEntry_GRec.SetRange("Variant Code", InvMovement_gQry.Variant_Code);
                        ...
                        (TotalRcptAmt = 0) and (PurchaseAmount = 0) and (OutputAmount = 0) and (PositiveAmount = 0) and (TransferRcptAmount = 0) and (SalesRtrnAmt = 0)
                        and (Closing = 0) and (ClosingAmount = 0)) then
    ...
    var
        ...
        InvMovement_gQry: Query "Inventory Movement";
        CurrInvMovement_gQry: Query "Inventory Movement";
    ...
    begin
        ...
        ExcelBuffer_gRecTmp.AddInfoColumn(CopyStr(InvMovement_gQry.GetFilters, 1, 250), false, false, false, false, '', ExcelBuffer_gRecTmp."cell type"::Text);
    ...
    begin
        Clear(Item_lRec);
        if Item_lRec.get(InvMovement_gQry.Item_No_) then;
        ExcelBuffer_gRecTmp.NewRow();
        ExcelBuffer_gRecTmp.AddColumn(InvMovement_gQry.Item_No_, false, '', false, false, false, '', ExcelBuffer_gRecTmp."cell type"::Text);
        ExcelBuffer_gRecTmp.AddColumn(InvMovement_gQry.Variant_Code, false, '', false, false, false, '', ExcelBuffer_gRecTmp."cell type"::Text);
        ExcelBuffer_gRecTmp.AddColumn(InvMovement_gQry.Location_Code, false, '', false, false, false, '', ExcelBuffer_gRecTmp."cell type"::Text);
