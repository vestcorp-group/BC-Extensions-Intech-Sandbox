report 50111 "Aged Accounts Receivable Peri"
{
    DefaultLayout = RDLC;
    RDLCLayout = './Layouts/AgedAccountsReceivablePeriodWise.rdl';
    ApplicationArea = Basic, Suite;
    Caption = 'Aged Accounts Receivable PeriodWise';
    ...
    dataset
    {
        dataitem(Header; "Integer")
        {
            ...
            column(HeaderText16; HeaderText[16])
            {
            }
            column(HeaderText15; HeaderText[15])
            {
            }
            ...
            dataitem(Company; Company)
            {
                dataitem(Customer; Customer)
                {
                    ...
                    //     begin
                    //         CustLedgEntry.ChangeCompany(CompanyNameG);
                    ...
                    //         CustLedgEntry.SetRange("Posting Date", StartingDt, EndingDate);
                    //         CopyDimFiltersFromCustomer(CustLedgEntry);
                    //         if CustLedgEntry.FindSet(false) then
                    //             repeat
                    //                 InsertTemp(CustLedgEntry, Company.Name)
                    ...
                    dataitem(OpenCustLedgEntry; "Cust. Ledger Entry")
                    {
                        ...
                        begin
                            ...
                            //090924-NE
                            InsertTemp(OpenCustLedgEntry, Company.Name);
                            CurrReport.Skip();
                        end;

                        trigger OnPreDataItem()
                        begin
                            OpenCustLedgEntry.ChangeCompany(Company.Name);

                            if AgingBy = AgingBy::"Posting Date" then begin
                                // SetRange("Posting Date", 0D, EndingDate);
                                // SetRange("Date Filter", 0D, EndingDate);
                                SetRange("Posting Date", StartingDt, EndingDate);
                                SetRange("Date Filter", StartingDt, EndingDate);
                                SetAutoCalcFields("Remaining Amt. (LCY)");
                                // SetFilter("Remaining Amt. (LCY)", '<>0');     //090924-O
                            end else begin
                                SetRange("Due Date", StartingDt, EndingDate);
                                SetRange("Date Filter", StartingDt, EndingDate);
                            end;
                    ...
                    dataitem(CurrencyLoop; "Integer")
                    {
                        ...
                        dataitem(TempCustLedgEntryLoop; "Integer")
                        {
                            ...
                            column(Team_gtxt; Team_gtxt)
                            {

                            }
                            column(CLEEndDocumentDate; Format(CustLedgEntryEndingDate."Document Date", 0, '<Day,2>/<Month,2>/<Year4>'))
                            {
                            }
                            ...
                            //VJ-NS
                            // column(InvoiceAmt_gDec; InvoiceAmt_gDec)
                            // {

                            // }
                            column(InvoiceAmt_gDec; CustLedgEntryEndingDate.Amount)
                            {

                            }
                            // column(InvoiceAmtLCY_gDec; InvoiceAmtLCY_gDec)
                            // {

                            // }
                            column(InvoiceAmtLCY_gDec; CustLedgEntryEndingDate."Amount (LCY)")
                            {

                            }
                            //VJ-NE
                            column(DueAmt_gDec; CustLedgEntryEndingDate."Remaining Amount")
                            {

                            }
                            column(DueAmtLCY_gDec; CustLedgEntryEndingDate."Remaining Amt. (LCY)")
                            {
                            ...
                            column(AgedCLE15RemAmtLCY; AgedCustLedgEntry[15]."Remaining Amt. (LCY)")
                            {
                                AutoFormatType = 1;
                            }
                            column(AgedCLE16RemAmtLCY; AgedCustLedgEntry[16]."Remaining Amt. (LCY)")
                            {
                                AutoFormatType = 1;
                            }
                            ...
                            column(CLEEndDueDate; Format(CustLedgEntryEndingDate."Due Date", 0, '<Day,2>/<Month,2>/<Year4>'))
                            {
                            }
                            ...
                            column(CLEPostingDate; Format(CustLedgEntryEndingDate."Posting Date", 0, '<Day,2>/<Month,2>/<Year4>'))
                            {
                            }
                            column(AgedCLE16TempRemAmt; AgedCustLedgEntry[16]."Remaining Amount")
                            {
                                AutoFormatExpression = CurrencyCode;
                                AutoFormatType = 1;
                            }
                            column(AgedCLE15TempRemAmt; AgedCustLedgEntry[15]."Remaining Amount")
                            {
                                AutoFormatExpression = CurrencyCode;
                                AutoFormatType = 1;
                            }
                            ...
                            column(BusPostGrp; Customer."Gen. Bus. Posting Group")
                            {
                            }
                            column(CustPostGrp; Customer."Customer Posting Group")
                            {
                            }
                            column(ControlAcc; CustomerPostingGroup_gRec."Receivables Account")
                            {

                            }
                            column(PaymentTermsCode; Customer."Payment Terms Code")
                            {

                            }
                            column(CreditLimitLCY; Customer."Credit Limit (LCY)")
                            {

                            }
                            column(InsuranceLimitLCY; Customer."Insurance Limit (LCY) 2")
                            {
                            ...
                            column(CurrrencyCode; DocumentCurr_gCod)  // CurrencyCode)
                            {
                                AutoFormatExpression = CurrencyCode;
                                AutoFormatType = 1;
                            }
                            ...
                            var
                                ...
                                SCH_lRec: Record "Sales Cr.Memo Header";
                                SCL_lRec: Record "Sales Cr.Memo Line";
                                SIL_lRec: Record "Sales Invoice Line";
                                SP_lRec: Record "Salesperson/Purchaser";
                                CurrExchRate_lRec: Record "Currency Exchange Rate";
                            begin
                                Customer.CalcFields("Inv. Amounts (LCY)", "Invoice Amounts");
                                ...
                                if AgingBy = AgingBy::"Due Date" then begin
                                    if CustLedgEntryEndingDate."Posting Date" > EndingDate then   //VJ_U
                                        currreport.Skip();
                                end;
                                ...
                                //CustLedgEntryEndingDate.CalcFields("Remaining Amount", "Remaining Amt. (LCY)");
                                if CustLedgEntryEndingDate."Remaining Amount" = 0 then
                                    CurrReport.Skip();

                                Clear(Salesperson_gTxt);
                                Clear(InvoiceAmt_gDec);
                                Clear(InvoiceAmtLCY_gDec);
                                Clear(SIH_lRec);
                                Clear(SCH_lRec);
                                Clear(DocumentCurr_gCod);
                                SIH_lRec.ChangeCompany(Company.Name);
                                SCH_lRec.ChangeCompany(Company.Name);
                                SCL_lRec.ChangeCompany(Company.Name);
                                DocumentCurr_gCod := CustLedgEntryEndingDate."Currency Code";
                                if SIH_lRec.get(CustLedgEntryEndingDate."Document No.") then begin
                                    Clear(SP_lRec);
                                    SP_lRec.ChangeCompany(Company.Name);
                                    if SP_lRec.get(SIH_lRec."Salesperson Code") then
                                        Salesperson_gTxt := SP_lRec."Name";
                                    Clear(SIL_lRec);
                                    SIL_lRec.ChangeCompany(Company.Name);
                                    SIL_lRec.SetRange("Document No.", SIH_lRec."No.");
                                    if SIL_lRec.FindSet() then begin
                                        repeat
                                            InvoiceAmt_gDec += SIL_lRec."Line Amount";
                                        until SIL_lRec.Next() = 0;
                                    end;

                                    if SIH_lRec."Currency Code" <> '' then begin
                                        if CustLedgEntryEndingDate."Adjusted Currency Factor" <> 0 then
                                            InvoiceAmtLCY_gDec := InvoiceAmt_gDec / CustLedgEntryEndingDate."Adjusted Currency Factor"
                                        else if SIH_lRec."Currency Factor" <> 0 then
                                            InvoiceAmtLCY_gDec := InvoiceAmt_gDec / SIH_lRec."Currency Factor";
                                    end else
                                        InvoiceAmtLCY_gDec := InvoiceAmt_gDec;
                                    if CustLedgEntryEndingDate."Currency Code" = '' then
                                        DocumentCurr_gCod := SIH_lRec."Currency Code";

                                end else if SCH_lRec.get(CustLedgEntryEndingDate."Document No.") then begin
                                    Clear(SP_lRec);
                                    SP_lRec.ChangeCompany(Company.Name);
                                    if SP_lRec.get(SCH_lRec."Salesperson Code") then
                                        Salesperson_gTxt := SP_lRec."Name";
                                    Clear(SCL_lRec);
                                    SCL_lRec.ChangeCompany(Company.Name);
                                    SCL_lRec.SetRange("Document No.", SCH_lRec."No.");
                                    if SCL_lRec.FindSet() then begin
                                        repeat
                                            InvoiceAmt_gDec += SCL_lRec."Line Amount";
                                        until SIL_lRec.Next() = 0;
                                    end;

                                    if SCH_lRec."Currency Code" <> '' then begin
                                        if CustLedgEntryEndingDate."Adjusted Currency Factor" <> 0 then
                                            InvoiceAmtLCY_gDec := InvoiceAmt_gDec / CustLedgEntryEndingDate."Adjusted Currency Factor"
                                        else if SCH_lRec."Currency Factor" <> 0 then
                                            InvoiceAmtLCY_gDec := InvoiceAmt_gDec / SCH_lRec."Currency Factor";
                                    end else
                                        InvoiceAmtLCY_gDec := InvoiceAmt_gDec;
                                    if CustLedgEntryEndingDate."Currency Code" = '' then
                                        DocumentCurr_gCod := SCH_lRec."Currency Code";
                                end;
                                ...
                                //if EndingDate >= CustLedgEntryEndingDate."Due Date" then
                                OverDueDays_gInt := EndingDate - CustLedgEntryEndingDate."Due Date";
                                ...
                                Clear(CustomerPostingGroup_gRec);
                                if CustomerPostingGroup_gRec.get(Customer."Customer Posting Group") then;

                                Clear(GLName_gTxt);
                                if GLAccount_gRec.Get(CustomerPostingGroup_gRec."Receivables Account") then
                                    GLName_gTxt := GLAccount_gRec.Name;

                                if SIH_lRec.get(CustLedgEntryEndingDate."Document No.") then begin
                                    Clear(Team_gtxt);
                                    TeamSP_gRec.Reset();
                                    TeamSP_gRec.SetRange("Salesperson Code", SIH_lRec."Salesperson Code");
                                ...
                                CurrExchRate_lRec.SetFilter("Starting Date", '..%1', EndingDate);
                                //CurrExchRate_lRec.SetFilter("Exchange Rate Amount", '<>%1', 0);
                                if CurrExchRate_lRec.FindLast() then begin
                                    AgedCustLedgEntry[PeriodIndex]."Remaining Amount" := 1 / CurrExchRate_lRec."Relational Exch. Rate Amount" * CustLedgEntryEndingDate."Remaining Amt. (LCY)";
                                end;
                    ...
                    begin
                        if VendorPostingGroup_gTxt <> '' then
                            Customer.SetFilter("Customer Posting Group", VendorPostingGroup_gTxt);
                    end;
                }
                ...
                begin
                    ...
                    GLSetup.ChangeCompany(Company.Name);
                    CustLedgEntryEndingDate.ChangeCompany(Company.Name);
                    ...
                    CustomerPostingGroup_gRec.ChangeCompany(Company.Name);
                    TempCustLedgEntry.ChangeCompany(Company.Name);
                end;
    ...
    var
        ...
        TotalCustLedgEntry: array[16] of Record "Cust. Ledger Entry";
        GrandTotalCustLedgEntry: array[16] of Record "Cust. Ledger Entry";
        AgedCustLedgEntry: array[16] of Record "Cust. Ledger Entry";
        ...
        CompanyNameG: Text;
        CustomerPostingGroup_gRec: Record "Customer Posting Group";
        OverDueDays_gInt: Integer;
        Salesperson_gTxt: Text;
        Team_gTxt: Text;
        Team_gRec: Record Team;
        TeamSP_gRec: Record "Team Salesperson";
        PrintAmountInLCY: Boolean;
        SkipZeroBalanceCustomers: Boolean;
        StartingDt, EndingDate : Date;
        AgingBy: Option "Due Date","Posting Date"; //,"Document Date";
        HeadingType: Option "Date Interval","Number of Days";
        NewPagePercustomer: Boolean;
        PeriodStartDate: array[16] of Date;
        PeriodEndDate: array[16] of Date;
        HeaderText: array[16] of Text[30];
        ...
        CompanyDisplayName, OverduedStyle_gTxt, VendorPostingGroup_gTxt, FilterComp_gTxt : Text;
    ...
    begin
        ...
        //if AgingBy = AgingBy::"Due Date" then begin
        PeriodStartDate[1] := StartingDt;
        for i := 1 to 16 do begin
            if i <> 1 then begin
                if PeriodEndDate[i - 1] >= EndingDate then
                    break;
                PeriodStartDate[i] := PeriodEndDate[i - 1] + 1;
            end;
            case
                Date2DMY(PeriodStartDate[i], 2) of
                1, 2, 3:
                    begin
                        //DMY2Date(01, 01, Date2DMY(StartingDt, 3));
                        if EndingDate > DMY2Date(31, 03, Date2DMY(PeriodStartDate[i], 3)) then begin
                            PeriodEndDate[i] := DMY2Date(31, 03, Date2DMY(PeriodStartDate[i], 3));
                            HeaderText[i] := '31 March ' + Format(Date2DMY(PeriodStartDate[i], 3));
                        end else begin
                            PeriodEndDate[i] := EndingDate;
                        end;
                    end;
                4, 5, 6:
                    begin
                        //PeriodStartDate[1] := DMY2Date(01, 04, Date2DMY(StartingDt, 3));
                        if EndingDate > DMY2Date(30, 06, Date2DMY(PeriodStartDate[i], 3)) then begin
                            PeriodEndDate[i] := DMY2Date(30, 06, Date2DMY(PeriodStartDate[i], 3));
                            HeaderText[i] := '30 June ' + Format(Date2DMY(PeriodStartDate[i], 3));
                        end else begin
                            PeriodEndDate[i] := EndingDate;
                            HeaderText[i] := Format(Date2DMY(EndingDate, 1)) + ' ' + GetMonthText_lFnc(Date2DMY(EndingDate, 2)) + ' ' + Format(Date2DMY(PeriodStartDate[i], 3));
                        end;
                    end;
                7, 8, 9:
                    begin
                        //PeriodStartDate[1] := DMY2Date(01, 07, Date2DMY(StartingDt, 3));
                        if EndingDate > DMY2Date(30, 09, Date2DMY(PeriodStartDate[i], 3)) then begin
                            PeriodEndDate[i] := DMY2Date(30, 09, Date2DMY(PeriodStartDate[i], 3));
                            HeaderText[i] := '30 September ' + Format(Date2DMY(PeriodStartDate[i], 3));
                        end else begin
                            PeriodEndDate[i] := EndingDate;
                            HeaderText[i] := Format(Date2DMY(EndingDate, 1)) + ' ' + GetMonthText_lFnc(Date2DMY(EndingDate, 2)) + ' ' + Format(Date2DMY(PeriodStartDate[i], 3));
                        end;
                ...
                10, 11, 12:
                    begin
                        //PeriodStartDate[1] := DMY2Date(01, 10, Date2DMY(StartingDt, 3));
                        if EndingDate > DMY2Date(31, 12, Date2DMY(PeriodStartDate[i], 3)) then begin
                            PeriodEndDate[i] := DMY2Date(31, 12, Date2DMY(PeriodStartDate[i], 3));
                            HeaderText[i] := '31 December ' + Format(Date2DMY(PeriodStartDate[i], 3));
                        end else begin
                            PeriodEndDate[i] := EndingDate;
                            HeaderText[i] := Format(Date2DMY(EndingDate, 1)) + ' ' + GetMonthText_lFnc(Date2DMY(EndingDate, 2)) + ' ' + Format(Date2DMY(PeriodStartDate[i], 3));
                        end;
        ...
        //     PeriodStartDate[i] := PeriodEndDate[i - 1] + 1;
        //     if EndingDate > DMY2Date(31, 12, Date2DMY(StartingDt, 3)) then
        //     PeriodEndDate[i] := 
        //     if PeriodStartDate[i] < EndingDate then begin
    ...
    local procedure InsertTemp(var CustLedgEntry: Record "Cust. Ledger Entry"; ComanyNameP: text)
    var
        Currency: Record Currency;
    begin
        TempCustLedgEntry.ChangeCompany(ComanyNameP);
    ...
    local procedure GetMonthText_lFnc(Month_iInt: Integer): Text
    begin
        case
            Month_iInt of
            1:
